...
def train_EVDI(opt):
    ...
    for i in range(len(opt.train_path)):
        current_dataset = train_dataset(opt.train_path[i], num_bins=16, num_frames=opt.num_frames, roi_size=(64,64)) 
    ...
    for epoch in range(opt.num_epoch):
        ...
        for i, (leftB_inp1,leftB_inp2,leftB,leftB_w1,leftB_w2, 
            ...
            res = res.reshape((B,N,color_channel,H,W))
            Ef1 = Ef1.reshape((B,N,color_channel,H,W))
            Ef2 = Ef2.reshape((B,N,color_channel,H,W))
            ...
            if (i+1) % opt.save_int == 0:
                 ...
                 show = torch.cat((leftB[0,...], rightB[0,...], res[0,idx,:]), 2).permute(1,2,0).squeeze()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_int", type=int, default=30, help="epoch interval for saving temporal reconstruction result")
    parser.add_argument("--train_path", type=list, default=["./Database/GoPro/train/"], help="path of training datasets")
    ...
    parser.add_argument("--num_frames", type=int, default=49, help="recover how many frames per input. This value is related to the parameter 'M' in paper.\
                        We observe that higher M leads to better performance but will need more training time (recommended num_frames>=25). ")
